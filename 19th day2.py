...
for i in dbase:
    print(i)
dbase.execute("CREATE TABLE Student7(Addmission_no VARCHAR(225),First_Name VARCHAR(20), Surname VARCHAR(20), DOB int(10), Gender VARCHAR(6) , Year int(3))")
dbase.execute("SHOW TABLES")
for table in dbase:
    print(table)
for row in range(1,sheet.utter_max_rows):
    sql = "INSERT INTO Student7(Addmission_no, First_Name, Surname, DOB, Gender, Year)VALUES (%s,%s,%s,%s,%s,%s)"
    for _ in range(0):
...
mydb.commit()
dbase.execute("SELECT * FROM Student7")
...
mycursor.execute('SELECT * FROM student7')
